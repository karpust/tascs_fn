...
from authapp.tasks import send_email_task, delete_unconfirmed_users
...
class TestSendEmailConfirmation(TestCase):
    ...
    def test_send_email_success(self):
        email_types = ("register_confirmation", "repeat_register_confirmation", "reset_password_confirmation")
        subject_types = ('Подтверждение email', 'Повторное подтверждение email', 'Сброс пароля')
        body_types = ("подтвердите свой email", "подтвердите свой email", "Перейдите по ссылке для сброса пароля")

        for i in range(len(email_types)):
            send_email_task(email_types[i], context={'user': self.user, 'confirmation_link': "some_link"}, recipient=self.user.email)
    ...
    @patch('authapp.tasks.EmailMultiAlternatives.send', side_effect=Exception("SMTP error"))
    def test_send_email_failure(self, mock_send, mock_logger):
        with patch.object(send_email_task, 'retry', side_effect=Retry('retry called')):
            with self.assertRaises(Retry):
                send_email_task('register_confirmation', context={'user': self.user, 'confirmation_link': "some_link"}, recipient=self.user.email)

        mock_send.assert_called_once()
        self.assertEqual(len(mail.outbox), 0)
        mock_logger.exception.assert_called_once_with("[FAILURE] Type: register_confirmation")
