...
class SingleGroupUserAdmin(UserAdmin):
    ...
    def save_model(self, request, obj, form, change):
        """
        Django хранит ManyToMany в отдельной промежуточной
        таблице (auth_user_groups).
        Чтобы добавить запись в эту таблицу, нужно знать user.id.
        А id появляется только после obj.save() — до этого объект существует
        только в памяти и obj.id = None.
        """
