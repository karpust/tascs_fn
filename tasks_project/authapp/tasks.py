...
def send_email_task(self, email_type, context, recipient):
    """
    выполняет задачи с разными приоритетами, на разных воркерах,
    принимает тип письма, по которому выбирает шаблон-html.
    на выполнение задачи дается 3 попытки c интервалом в 5 мин
    """
    logger.info(f"[SEND EMAIL TASK CODE STARTED]")
    try:
        ...
        # logger.error(f"[FAILURE] Type: {email_type} | Email: {recipient} | Error: {str(e)}")
