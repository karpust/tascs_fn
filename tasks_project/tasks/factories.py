...
from tasks.models import Task, Tag, Category, Comment
...
class TaskFactory(factory.django.DjangoModelFactory):
    ...
    status = factory.LazyFunction(lambda: random.choice([status.value for status in TaskStatus]))
    deadline = factory.LazyFunction(lambda: timezone.now() + timedelta(days=random.randint(-3, 3)))
    priority = factory.LazyFunction(lambda: random.choice([p.value for p in TaskPriority]))
    ...
    def category(self, create, extracted, **kwargs):
        ...
        if extracted:
            ...
            # category_list = ["Feature", "Improvement", "Documentation", "Research", "Testing"]
...
class CommentFactory(factory.django.DjangoModelFactory):
    ...
    logger.info(f'Start to create CommentFactory class')
    text = Faker('sentence')
    created_at = factory.LazyFunction(lambda: timezone.now() + timedelta(days=random.randint(-3, 3)))
