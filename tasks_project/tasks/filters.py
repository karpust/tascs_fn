...
from .models import Task, Tag, TaskStatus, TaskPriority
...
class TaskFilter(django_filters.FilterSet):
    """
    Фильтрация по status_display(строке), а сортировка по status(числу) - для удобства и функциональности.
    """
    # custom:
    ...
    executor = django_filters.CharFilter(field_name="executor__username", lookup_expr="iexact")
    ...
    status_display = django_filters.ChoiceFilter(
        ...
        # ==> [('to_do', 'to_do'),('in_progress', 'in_progress'),('done', 'done')] ==> ChoiceFilter ==> filter_by_status_display
    ...
    class Meta:
        model = Task
        fields = ['status_display', 'priority_display',
                  "deadline_after", "deadline_before", "owner",
                  'executor', "tags", "search", "category"]  # here exact or custom
    ...
    def filter_by_status_display(self, queryset, name, value):
        return self.filter_by_field_display(queryset, name, value, "status", TaskStatus.choices)

    def filter_by_prioriry_display(self, queryset, name, value):
        return self.filter_by_field_display(queryset, name, value, "priority", TaskPriority.choices)