...
from tasks.models import Task, Comment, Category, Tag
...
class BaseTaskModelTests(TestCase):
    """
    родительский класс для наследования от него
    методов setUpTestData, setUp
    """
    ...
    def setUpTestData(cls):
        ...
        созданные объекты не сбрасываются автоматически,
        использую для ускорения тестов
        """
        ...
        cls.comment = Comment.objects.create(task=cls.task, author=cls.user, text='Test comment')
