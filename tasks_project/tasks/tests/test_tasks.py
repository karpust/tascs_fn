...
from tasks.tasks import left_time, deadline_notification
...
class TestNotificationTask(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.executor1 = User.objects.create(username='executor1', email='executor1@mail.com')
        cls.executor2 = User.objects.create(username='executor2', email='executor2@mail.com')
        cls.executor3 = User.objects.create(username='executor3', email='executor3@mail.com')
        cls.executor4 = User.objects.create(username='executor4', email='executor4@mail.com')
        ...
        cls.task1 = Task.objects.create(owner=cls.owner1,
                                        title='task1',
                                        notified=False,
                                        deadline=timezone.now() + timedelta(hours=left_time))  # нужно уведомить
        cls.task1.executor.set([cls.executor1])
        cls.task2 = Task.objects.create(owner=cls.owner2,
                                        notified=False,
                                        title='task2',
                                        deadline=timezone.now() + timedelta(hours=1))  # нужно уведомить
        cls.task2.executor.set([cls.executor2, cls.executor3])
        cls.task3 = Task.objects.create(owner=cls.owner3,
                                        title='task3',
                                        notified=False,
                                        deadline=timezone.now() + timedelta(hours=left_time+6))  # рано уведомлять
        cls.task3.executor.set([cls.executor3])
        cls.task4 = Task.objects.create(owner=cls.owner4,
                                        title='task4',
                                        notified=False,
                                        deadline=timezone.now() - timedelta(hours=1))  # просрочена
        cls.task4.executor.set([cls.executor4])
        cls.task5 = Task.objects.create(owner=cls.owner4,
                                        title='task5',
                                        notified=True,
                                        deadline=timezone.now() + timedelta(hours=15))  # уже уведомили
    ...
    @patch("tasks.tasks.EmailMultiAlternatives.send", side_effect=Exception("SMTP error"))
    def test_exeptions(self, mock_send, mock_logger):
        with patch.object(deadline_notification, 'retry', side_effect=Retry("retry called")) as mock_retry:
        ...
        mock_logger.exception.assert_called_once_with("[FAILURE] Type: Deadline notifications")

    def test_deadline_soon_not_notified(self):
        """
        тест задачи с близким дедлайном(24ч),
        о котором еще не было уведомления
        ...
        for email in mail.outbox:
            self.assertIn("Скоро дедлайн выполняемой вами задачи"
                          if email.to[0].startswith('executor') else
                          "Скоро дедлайн созданной вами задачи", email.subject)
