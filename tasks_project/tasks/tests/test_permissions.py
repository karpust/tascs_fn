...
class TaskPermissionTestCase(BasePermissionTestCase):
    ...
    мокаю объекты на случай если изменятся
    и начнут требовать обязательные поля
    """
    @classmethod
    def setUpTestData(cls):
        super().setUpTestData()
    ...
    def test_task_all_by_admin(self, mock_is_admin):
        ...
        for method in allowed_methods:
            ...
            self.assertTrue(self.permission.has_permission(request, None))  # у админа доступ к api
            # view=None, не нужен, т к проверяю только пермишен: по юзеру и методу запроса
    ...
    def test_task_read_by_authorized(self, mock_is_user):
        ...
        request.data = {"status": "in_progress", "desctiption": "Working on it"}
    ...
    def test_executor_can_update_status_and_comments(self, mock_is_user):
        ...
        self.assertTrue(self.permission.has_object_permission(request, None, self.task))  # можно изменять дозволенные поля
        ...
        self.assertTrue(self.permission.has_permission(request, None))  # юзеру доступен api
        self.assertFalse(self.permission.has_object_permission(request, None, self.task))  # нельзя изменять запрещенные поля
...
class CommentPermissionTestCase(BasePermissionTestCase):
    ...
    проверить что:
    админ может все;
    коммент к задачам может:
        создавать owner, executor этой задачи,
        редактировать частично, полностью, удалять author,
        просматривать любой авторизованный

    коммент к задачам не может:
        создавать owner, executor другой задачи, авторизованный,
